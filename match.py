...
def update_team_rating(leaderboard_sheet, team_name, won, elo_win, elo_loss):
    ...
    # If team not found (unlikely), create new entry
...
async def generate_weekly_matches(interaction, spreadsheet, week_number, force=False):
    ...
    # Handle outstanding matches if forcing
    if force:
        existing_matches = matches_sheet.get_all_values()[1:]

        for idx, row in enumerate(existing_matches, start=2):
            match_id, team_a, team_b, _, _, status, _, _, winner, loser, _ = row

            if status.strip() == "" or status.strip().lower() not in ["finished", "cancelled"]:
                team_a_players = team_player_counts.get(team_a, 0)
                team_b_players = team_player_counts.get(team_b, 0)

                if team_a_players >= 3 and team_b_players >= 3:
                    matches_sheet.update_cell(idx, 5, "Finished")
                    matches_sheet.update_cell(idx, 6, "Double Forfeit")
                    matches_sheet.update_cell(idx, 9, "N/A")
                    matches_sheet.update_cell(idx, 10, "N/A")

                elif team_a_players < 3 and team_b_players >= 3:
                    winner_team = team_b
                    loser_team = team_a
                    matches_sheet.update_cell(idx, 5, "Finished")
                    matches_sheet.update_cell(idx, 6, "Forfeited")
                    matches_sheet.update_cell(idx, 9, winner_team)
                    matches_sheet.update_cell(idx, 10, loser_team)

                    if affect_elo:
                        update_team_rating(leaderboard_sheet, winner_team, True, elo_win, elo_loss)
                        update_team_rating(leaderboard_sheet, loser_team, False, elo_win, elo_loss)

                elif team_b_players < 3 and team_a_players >= 3:
                    winner_team = team_a
                    loser_team = team_b
                    matches_sheet.update_cell(idx, 5, "Finished")
                    matches_sheet.update_cell(idx, 6, "Forfeited")
