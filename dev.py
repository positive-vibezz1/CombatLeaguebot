...
class DevPanel_Match(SafeView):
    ...
    async def force_weekly(self, interaction, button):
        class ForceWeeklyMatchups(Modal, title="Force Weekly Matchups"):
            week = TextInput(label="Week Number", required=True)
            def __init__(self, parent): super().__init__(); self.parent = parent
            async def on_submit(self, i):
                import match
                await self.parent.safe_send(i, "✅ Generating weekly matches...")
                await match.generate_weekly_matches(i, self.parent.spreadsheet, int(self.week.value), force=True)
    ...
    async def announce_unscheduled(self, interaction, button):
        ...
        with open("config.json") as f:
            config = json.load(f)
        channel = interaction.guild.get_channel(int(config.get("match_channel_id")))
        sheet = get_or_create_sheet(self.spreadsheet, "Matches", ["Match ID","Team A","Team B","Proposed Date","Scheduled Date","Status","Winner","Loser","Proposed By"])
        for row in sheet.get_all_values()[1:]:
            if row[4] in ["", "TBD"] and row[5] not in ["Finished", "Cancelled", "Forfeited"]:
                await channel.send(f"📢 **Unscheduled Match:** {row[1]} vs {row[2]}")
        await interaction.followup.send("✅ Announced unscheduled matches.", ephemeral=True)
...
class DevPanel_System(SafeView):
    ...
    async def lock_rosters(self, interaction, button):
        ...
        for idx in range(2, len(v) + 1):
            s.update_cell(idx, len(v[0]) + 1, "Yes")
