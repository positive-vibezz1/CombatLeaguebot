...
async def create_matchup_channel(interaction, team_a, team_b, captain_a_id, captain_b_id, week_number):
    ...
    # Add captain permissions if they exist
...
async def generate_weekly_matches(interaction, spreadsheet, week_number, force=False):
    ...
    team_names = [team[0] for team in all_teams]
    matchups = []
    used_pairs = set()
    ...
    for idx, team in enumerate(team_names):
        ...
        for opp_idx, opponent in enumerate(team_names):
            if team == opponent:
                continue
        ...
        if len(opponents) < 2 and not force:
            await interaction.followup.send(f"Not enough available teams to generate full 2 matches for {team}.", ephemeral=True)
            return
